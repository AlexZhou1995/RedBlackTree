...
public class Tree {
	...
	public String print(Node now,String result) {
		if(now.isReal){
			String r=print(now.left,result);
			//System.out.print(now.value+" "+now.isRed+" "+now.time);
			r=r+"value:"+Double.toString(now.value)+" ";
			if (now.isRed){
				r=r+"red ";
			}else {
				r=r+"black ";
			}
			r=r+"Appears:"+Integer.toString(now.time)+" ";
			if(now.parent!=null){
				//System.out.println(" "+now.parent.isRed+" "+now.parent.value);
				String text=now.parent.isRed?  "red":"black";
				r=r+"Parent:"+text+" "+Double.toString(now.parent.value)+"\n\r";
			}else {
				//System.out.println();
				r=r+"\n\r";
			}
			return print(now.right,r);
		}else{
			return result;
		}
	}
